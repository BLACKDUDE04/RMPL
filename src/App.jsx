...
function PlayersPage({ refreshData }) {
  ...
  const filtered = players.filter((player) => {
    ...
    const matchesSearch = [player.auctionNumber, player.name, player.age, player.category, player.playedIn, player.team, player.details, player.cricheroesId, player.sold ? 'sold' : player.unsold ? 'unsold' : 'available'].join(' ').toLowerCase().includes(query.toLowerCase());
  ...
  return <main className="panel">
    ...
    {showForm || editing ? <form className="manual-player-form player-editor form-reveal" onSubmit={save} key={editing?._id || 'new'}>
      ...
      <label>CricHeroes ID<input name="cricheroesId" type="number" inputMode="numeric" min="0" step="1" defaultValue={editing?.cricheroesId || ''} placeholder="Optional numeric ID" /></label>
  ...
  return (
    <main className="panel registration-page">
      ...
      <form ref={formRef} className="registration-form" onSubmit={handleSubmit} aria-busy={submitting}>
        ...
        <label>CricHeroes ID<input name="cricheroesId" type="number" inputMode="numeric" min="0" step="1" placeholder=" Optional numeric player ID" /></label>
            ...
            return (
              <article className="registration-data-card" key={player._id}>
                ...
                <div className="registration-data-details">
                  ...
                  {player.cricheroesId ? <p><strong>CricHeroes:</strong> <a href={`https://cricheroes.com/player-profile/${player.cricheroesId}/player/profile`} target="_blank" rel="noreferrer">Open player profile</a></p> : null}
